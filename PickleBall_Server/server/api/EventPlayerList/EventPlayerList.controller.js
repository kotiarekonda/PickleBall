...
exports.eventbasedplayers = function(req, res){
  EventPlayerList.find({TournamentId: req.user._id, EventId: req.body.EventId, Selected: false})
  .populate({path:'Player1Id', select:'FirstName _id'})
  .populate({path:'Player2Id', select:'FirstName _id'})
  .lean()
  .exec(function(err, playeslist){
    ...
    if(playeslist.length > 0){
      ...
      playeslist.forEach(function(element, index){
        ...
        element.Player1Name = element.Player1Id.FirstName;
        element.Player1Id = element.Player1Id._id;
        if(element.Player2Id !== undefined){
          element.Player2Name = element.Player2Id.FirstName;
